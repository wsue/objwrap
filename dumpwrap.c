...
static const int dump_symtab;			/* -t */
static const int dump_dynamic_symtab;		/* -T */
static const int dump_reloc_info;		/* -r */
static const int dump_dynamic_reloc_info;	/* -R */
//static int dump_ar_hdrs;		/* -a */
//static int dump_private_headers;	/* -p */
static const int prefix_addresses;		/* --prefix-addresses */
static const int with_line_numbers  = 1;		/* -l */
static const bfd_boolean with_source_code;	/* -S */
static const int show_raw_insn;		/* --show-raw-insn */
static const int dump_dwarf_section_info;	/* --dwarf */
//static int dump_stab_section_info;	/* --stabs */
//static int do_demangle;			/* -C, --demangle */
static bfd_boolean disassemble      = 1;		/* -d */
static const bfd_boolean disassemble_all;	/* -D */
static const int disassemble_zeroes;		/* --disassemble-zeroes */
//static bfd_boolean formats_info;	/* -i */
static const int wide_output;			/* -w */
static const int insn_width;			/* --insn-width */
static const bfd_vma start_address = (bfd_vma) -1; /* --start-address */
static const bfd_vma stop_address = (bfd_vma) -1;  /* --stop-address */
static const int dump_debugging;		/* --debugging */
static const int dump_debugging_tags;		/* --debugging-tags */
...
//static const char *disassembler_options = NULL;
...
struct dumpwrap_ctl{
    const char              *filename;      //  当前操作文件名
    dasm_addr_t            vma_offset;     //  elf符号在库中偏移

    int                     allowout;       //  是否充许输出

    DASM_DUMP_CALLBACK     dump;
    DASM_DUMPFMT_CALLBACK  err;
    DASM_ACT_CHECK         act;

    void                    *param;
};

static  struct dumpwrap_ctl *s_pctl;
static inline void wrap_putchar(char    c){     
    if( s_pctl->allowout && s_pctl->dump )
    s_pctl->dump(s_pctl->param,&c,1);  
}


static int wrap_fprintf(struct dumpwrap_ctl *pctl, const char *fmt, ...)
{
    if( pctl && pctl->allowout && pctl->dump ){
        char    cache[256];
        va_list ap;    
        va_start (ap, fmt);
        int len = vsnprintf(cache,sizeof(cache),fmt,ap);
        va_end(ap);
        pctl->dump(pctl->param,cache,len);
        return len;
    }
    else
        return strlen(fmt);
...
#define wrap_fprintf_out                s_pctl

#define wrap_printf(fmt,arg...)         wrap_fprintf(s_pctl,fmt,##arg)

#define wrap_errout(fmt,arg...)     do{ if( s_pctl && s_pctl->err)  \
    s_pctl->err(s_pctl->param,"%s:%d(%s) "fmt,__func__,__LINE__,s_pctl->filename,##arg);  }while(0)
...
#define process_section_p(x)    TRUE
#if 0
static bfd_boolean
...
slurp_symtab (bfd *abfd)
{
  ...
  if (storage < 0){
      wrap_errout("get_symtab_upper_bound:%ld\n",storage);
  ...
  if (symcount < 0){
      wrap_errout("%s bfd_canonicalize_symtab:%ld\n",__func__,symcount);
    //bfd_fatal (bfd_get_filename (abfd));
      free(sy);
      symcount = 0;
      return NULL;
...
slurp_dynamic_symtab (bfd *abfd)
{
  ...
  if (storage < 0)
	...
	{
            wrap_errout(" bfd_canonicalize_symtab dyn:%ld\n",storage);
	  //non_fatal (_("%s: not a dynamic object"), bfd_get_filename (abfd));
	  dynsymcount = 0;
	  return NULL;
	}

            wrap_errout(" bfd_canonicalize_symtab:%ld\n",storage);
      //bfd_fatal (bfd_get_filename (abfd));
	  dynsymcount = 0;
	  return NULL;
    }
  ...
  if (dynsymcount < 0){
      wrap_errout(" bfd_canonicalize_dynamic_symtab:%ld\n",dynsymcount);
    //bfd_fatal (bfd_get_filename (abfd));
        free(sy);
      dynsymcount = 0;
	  return NULL;
...
static void
objdump_print_symname (bfd *abfd ATTRIBUTE_UNUSED, struct disassemble_info *info,
		       asymbol *sym)
{
  char *alloc;
  const char *name;
      ...
      else
	{
	  ...
	  if (! prefix_addresses)
		...
		*s = ' ';
	      if (*s == '\0')
		*--s = '0';
	      wrap_printf ("%s:\t", buf + skip_addr_chars);
	    }
	  else
	    {
	  ...
	  if (insns)
		  ...
		  && *relppp < relppend)
		{
	      ...
	      info->fprintf_func = (fprintf_ftype) wrap_fprintf;
	      info->stream = wrap_fprintf_out;
	  ...
	  if (prefix_addresses
	      ...
	      for (j = addr_offset * opb; j < addr_offset * opb + pb; j += bpc)
		{
		  ...
		  if (bpc > 1 && info->display_endian == BFD_ENDIAN_LITTLE)
		    {
		      for (k = bpc - 1; k >= 0; k--)
			wrap_printf ("%02x", (unsigned) data[j + k]);
		      wrap_putchar (' ');
		    }
		  else
		    {
  ...
  if (paux->dynrelbuf)
	  ...
	  && (dump_reloc_info || pinfo->disassembler_needs_relocs))
	{
	  ...
	  relsize = bfd_get_reloc_upper_bound (abfd, section);
            if (relsize < 0){
                wrap_errout(" bfd_get_reloc_upper_bound:%ld\n",relsize);
                //  bfd_fatal (bfd_get_filename (abfd));
                return ;
            }
	  ...
	  if (relsize > 0)
	    {
	      ...
	      rel_count = bfd_canonicalize_reloc (abfd, section, rel_pp, syms);
                if (rel_count < 0){
                    wrap_errout(" bfd_canonicalize_reloc:%ld\n",rel_count);
                    //bfd_fatal (bfd_get_filename (abfd));
                    goto exit;
                }
  ...
  ((SYM)->section == section \
   ...
   && pinfo->symbol_is_valid (SYM, pinfo))
	...
	insns = FALSE;
      ...
      if( !s_pctl->act || 
              (s_pctl->allowout = s_pctl->act(s_pctl->param,EDumpWrapFuncStart,
                                              s_pctl->filename,s_pctl->vma_offset,
                                              sym ? sym->name : NULL,
                                              addr,section->vma + nextstop_offset) ) != 0 ){
          ...
          if( s_pctl->act )
              s_pctl->act(s_pctl->param,EDumpWrapFuncEnd,
                      s_pctl->filename,s_pctl->vma_offset,
                      sym ? sym->name : NULL,
                      addr,section->vma + nextstop_offset);
      }
...
disassemble_data (bfd *abfd)
{
  ...
  init_disassemble_info (&disasm_info, wrap_fprintf_out, (fprintf_ftype) wrap_fprintf);
  ...
  if (!aux.disassemble_fn)
    {
                wrap_errout(" Can't disassemble for architecture:%s\n",bfd_printable_arch_mach (bfd_get_arch (abfd), 0));
      //non_fatal (_("Can't disassemble for architecture %s\n"),
		 //bfd_printable_arch_mach (bfd_get_arch (abfd), 0));
      //exit_status = 1;
      return BFDWRAP_ERR_DISASM_NOARCH;
    }

  disasm_info.flavour = bfd_get_flavour (abfd);
  disasm_info.arch = bfd_get_arch (abfd);
  disasm_info.mach = bfd_get_mach (abfd);
  disasm_info.disassembler_options = NULL;//disassembler_options;
		  ...
		  void *arg)
{
  if ((section->flags & SEC_DEBUGGING) == 0)
    {
        bfd_vma     *param      = (bfd_vma *)arg;
      bfd_boolean has_reloc_p = param[0];
      section->vma += param[1];
      if (has_reloc_p)
	section->lma += param[1];
    }
...
static void
dump_bfd (bfd *abfd,bfd_vma adjust_section_vma)
{
    int ret = 0;
  ...
  if (adjust_section_vma != 0)
    {
      bfd_boolean has_reloc = (abfd->flags & HAS_RELOC);
        bfd_vma param[2];
        param[0]        = has_reloc;
        param[1]        = adjust_section_vma;
      bfd_map_over_sections (abfd, adjust_addresses, param);
    }
...
static int
display_bfd (bfd *abfd,bfd_vma adjust_section_vma )
{
  char **matching;

  if (bfd_check_format_matches (abfd, bfd_object, &matching))
    {
      dump_bfd (abfd,adjust_section_vma);
      return 0;
    }

  if (bfd_get_error () == bfd_error_file_ambiguously_recognized)
    {
        wrap_errout(" bfd_error_file_ambiguously_recognized\n");
#if 0
      nonfatal (bfd_get_filename (abfd));
      list_matching_formats (matching);
#endif
      free (matching);
    return BFDWRAP_ERR_FILE_MULTIRECOG;
    }

  if (bfd_get_error () != bfd_error_file_not_recognized)
    {
        wrap_errout(" bfd_error %d\n",bfd_get_error ());
      //    nonfatal (bfd_get_filename (abfd));
        return BFDWRAP_ERR_FILE_UNSUPPORT;
    }
...
static int
display_file (char *filename,int fsize, char *target,bfd_vma adjust_section_vma)
{
  bfd *file;
  bfd *arfile = NULL;
  int       ret = 0;
  int       realsize    = get_file_size (filename);

  if ( realsize < 1 || (fsize != 0 && fsize != realsize ) )
    {
        wrap_errout(" get_size err or fsize %d not match %d\n",realsize ,fsize);
      return BFDWRAP_ERR_FILE_SZ;
    }

  file = bfd_openr (filename, target);
  if (file == NULL)
    {
        wrap_errout(" bfd_open err\n");
      //    nonfatal (filename);
      return BFDWRAP_ERR_FILE_OPEN;
    }
  ...
  if (bfd_check_format (file, bfd_archive))
	  ...
	  if (arfile == NULL)
	    {
	      if (bfd_get_error () != bfd_error_no_more_archived_files){
                  wrap_errout(" bfd_open_archie  err %d\n",bfd_get_error());
		//nonfatal (bfd_get_filename (file));
                    ret = BFDWRAP_ERR_FILE_OPEN;
              }
	      break;
	    }

	  ret   = display_bfd (arfile,adjust_section_vma);
    ...
    ret = display_bfd (file,adjust_section_vma);
        ...
        )
{
  ...
  if (formats_info)
    ...
    {
      if (optind == argc)
	display_file ("a.out",0, target);
      else
	for (; optind < argc;)
	  display_file (argv[optind++], 0,target);
    }
...
int dumpwrap_load(
        ...
        void *param )
{
    struct dumpwrap_ctl ctl;

    if( !fname || !*fname )
        return BFDWRAP_ERR_ARG;
    ...
    s_pctl      = &ctl;
    if( ctl.act )
        ctl.act(param,EDumpWrapBegin,
                fname,vma_offset,
                NULL,0,0);
    int ret = display_file ((char *)fname, fsize,NULL,0);
    if( ctl.act )
        ctl.act(param,EDumpWrapEnd,
                fname,vma_offset,
                NULL,0,0);
